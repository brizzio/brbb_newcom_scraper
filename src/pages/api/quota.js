...
export default async function handler(req, res) {
  ...
  try {
    ...
    const flattened = {
      cota_id: cota,
      ...flattenObject(consorciado),
      ...flattenObject(valoresReceber, 'valoresReceber'),
      ...flattenObject(valoresDevolver, 'valoresDevolver'),
    };

    return res.status(200).json(flattened);

  } catch (err) {
    console.error('Erro na rota /api/quota:', err);
